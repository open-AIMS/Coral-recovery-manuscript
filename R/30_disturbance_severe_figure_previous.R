...
## ---- Cyclones
{
  ## ---- cycloneModel
  {
    cyclones.full = all.reefs %>%
     ...
     mutate(
      ...
      library(mgcv)
      cyclones.any.gamm = gamm(CYCLONEany ~ s(REPORT_YEAR, by = Zone),
        random = list(REEF_NAME = ~1),
  ...
  ## ---- CyclonesCat2
  {
    ...
    d1 = plotEffects(cyclones.cat2.glmmTMB,
      firstYear = min(cyclones.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Blues")(4)[4]
    )
...
## ---- COTS
{
  ...
  ## ---- CotsAny
  {
    ...
    d1 = plotEffects(cots.any.glmmTMB,
      firstYear = min(cots.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Greens")(3)[3]
    )
  ...
  ## ---- CotsAnyPoly
  {
    cots.any.glmmTMB3 = glmmTMB(COTSany ~ poly(time,3,raw=FALSE)*Zone +
                                  (1|REEF_NAME) + ar1(-1+factor(time)|REEF_NAME),
      data=cots.binary, family=binomial())
    ...
    d1 = plotEffects(cots.any.glmmTMB3,
      firstYear = min(cots.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Greens")(3)[3]
    )
  ...
  ## ---- CotsSeverePoly
  {
    ...
    d1 = plotEffects(cots.severe.glmmTMB3,
      firstYear = min(cots.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Greens")(3)[3]
    )
  ...
  ## ---- CotsCatAOPoly
  {
    ...
    d1 = plotEffects(cots.catAO.glmmTMB3,
      firstYear = min(cots.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Greens")(3)[3]
    )
...
## ---- bleaching
{
  ...
  ## ---- BleachingAny
  {
    ...
    d1 = plotEffects(bleaching.any.glmmTMB,
      firstYear = min(bleaching.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Reds")(5)[5]
    )
  ...
  ## ---- BleachingAnyPoly
  {
    ...
    d1 = plotEffects(bleaching.any.glmmTMB3,
      firstYear = min(bleaching.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Reds")(5)[5]
    )
  ...
  ## ---- Bleaching severe - This is the one used..
  {
    ...
    d1 = plotEffects(bleaching.severe.glmmTMB,
      firstYear = min(bleaching.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Reds")(5)[5], points = FALSE
    )
  ...
  ## ---- Bleaching severe version 2 (exclude LTMP not in summer) - This is the one used..
  {
    ...
    dist.table2[["Bleaching"]][["Intercept"]] = calcFreqs(bleaching.severe.2.glmmTMB)[[1]] %>%
      dplyr::select(-time) %>%
      mutate(Disturbance = "Bleaching", Stat = "Intercept") %>%
      dplyr::select(Disturbance, Stat, everything())
    dist.table2[["Bleaching"]][["Slope"]] = calcFreqs(bleaching.severe.2.glmmTMB)[[2]] %>%
      mutate(Disturbance = "Bleaching", Stat = "Slope") %>%
      dplyr::select(Disturbance, Stat, everything())
    df=modelEachReef(bleaching.severe.2.glmmTMB,form=BLEACHINGsevere~time)
    d1 = plotEffects(bleaching.severe.2.glmmTMB,
      firstYear = min(bleaching.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Reds")(5)[5], points = FALSE
    )
  ...
  ## ---- BleachingSeverePoly
  {
    ...
    d1 = plotEffects(bleaching.severe.glmmTMB3,
      firstYear = min(bleaching.binary$REPORT_YEAR),
      individualReefs = df, brewer_pal(palette = "Reds")(5)[5]
    )
...
## ---- Collect together
{
  ## ---- Save the disturbance tables
  {
    ...
    write.csv(do.call("rbind", lapply(dist.table, `[[`, "Intercept")),
      file = "../data/modelled/dist.table.intercepts.csv", quote = FALSE, row.names = FALSE
    )
    write.csv(do.call("rbind", lapply(dist.table, `[[`, "Slope")),
      file = "../data/modelled/dist.table.slopes.csv", quote = FALSE, row.names = FALSE
    )
    write.csv(do.call("rbind", lapply(dist.table, `[[`, "PercentChange")),
      file = "../data/modelled/dist.table.percentchange.csv", quote = FALSE, row.names = FALSE
    )
    write.csv(do.call("rbind", lapply(dist.table, `[[`, "InterceptProb")),
      file = "../data/modelled/dist.table.interceptProb.csv", quote = FALSE, row.names = FALSE
    )
  }
  ## Combine all disturbances
  {
    disturb.binary = cyclones.binary %>%
      dplyr::select(REEF_ID,REPORT_YEAR,Zone,CYCLONEany,CYCLONEsevere) %>%
      full_join(cots.binary %>%
                  dplyr::select(REEF_ID,REPORT_YEAR,Zone,COTSany, COTSsevere)) %>%
      full_join(bleaching.binary %>%
                  dplyr::select(REEF_ID,REPORT_YEAR,Zone,BLEACHINGany, BLEACHINGsevere) %>%
                  mutate(REEF_ID=as.numeric(as.character(REEF_ID)))) %>%
      distinct

    disturb.binary <- disturb.binary %>%
      mutate(across(c(-Zone, -REPORT_YEAR, -REEF_ID), function(x) ifelse(is.na(x), 0, x)))
    disturb.binary = disturb.binary %>%
      filter(!is.na(CYCLONEany), !is.na(BLEACHINGany), !is.na(COTSany)) %>%
  ...
  ## ---- Save the disturbance tables version 2
  {
    ...
    write.csv(do.call("rbind", lapply(dist.table2, `[[`, "Intercept")),
      file = "../data/modelled/dist.table.2.intercepts.csv", quote = FALSE, row.names = FALSE
    )
    write.csv(do.call("rbind", lapply(dist.table2, `[[`, "Slope")),
      file = "../data/modelled/dist.table.2.slopes.csv", quote = FALSE, row.names = FALSE
    )

    ## Combine all disturbances
    disturb.binary.2 = cyclones.binary %>%
      dplyr::select(REEF_ID,REPORT_YEAR,Zone,CYCLONEany,CYCLONEsevere) %>%
      full_join(cots.binary %>%
                  dplyr::select(REEF_ID,REPORT_YEAR,Zone,COTSany, COTSsevere)) %>%
      full_join(bleaching.binary.2 %>%
                  dplyr::select(REEF_ID,REPORT_YEAR,Zone,BLEACHINGany, BLEACHINGsevere) %>%
                  mutate(REEF_ID=as.numeric(as.character(REEF_ID)))) %>%
      distinct()
...
{
  {
    extract_stats <- function(tab) {
      data.frame(tab$Intercept$Zone,
        Intecept = sprintf("%0.3g (%0.3g, %0.3g)", tab$Intercept$prob,
          tab$Intercept$lower.CL, tab$Intercept$upper.CL),
        Slope = sprintf("%0.3g (%0.3g, %0.3g)",tab$Slope$time.trend,
          tab$Slope$lower.CL, tab$Slope$upper.CL),
...
## Split up figure for Cell Press format (2020) ---------------------------------
  ...
  ## ---- Disturbances_severe_compilation_new_transposed.pdf
  {
    ...
    g.severe.bleaching3 <- with(g.severe.bleaching3$layout[facets,],
      gtable_add_grob(g.severe.bleaching3, ggplotGrob(banner_thumb_northern_v),t=6, l=10, b=8, r=10, name="pic_predator"))

    facets <- grep("strip-r-2", g.severe.bleaching3$layout$name)
    ## facets1 <- grep("strip-r-2", gg$layout$name)
    ## g.severe.bleaching3$grobs[facets] = gg$grobs[facets1]
    g.severe.bleaching3 <- with(g.severe.bleaching3$layout[facets,],
      gtable_add_grob(g.severe.bleaching3, ggplotGrob(banner_thumb_central_v),t=10, l=10, b=10, r=10, name="pic_predator"))
    facets <- grep("strip-r-3", g.severe.bleaching3$layout$name)
    ## facets1 <- grep("strip-r-3", gg$layout$name)
    ## g.severe.bleaching3$grobs[facets] = gg$grobs[facets1]
    g.severe.bleaching3 <- with(g.severe.bleaching3$layout[facets,],
      gtable_add_grob(g.severe.bleaching3, ggplotGrob(banner_thumb_southern_v),t=12, l=10, b=12, r=10, name="pic_predator"))
    g.severe.bleaching3 %>% grid.draw()
